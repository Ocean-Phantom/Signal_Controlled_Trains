...
function add_surface(surface)
	...
	if not global.Surfaces[index] then
		...
		for _, stop in pairs(global.Surfaces[index].Train_Stops) do
			add_train_stop(stop)
		end
		for item, _ in pairs(game.item_prototypes) do
			global.Surfaces[index].Demand_Stops_by_Signal.item[item] = {}
			global.Surfaces[index].Supply_Stops_by_Signal.item[item] = {}
		end
		for fluid, _ in pairs(game.fluid_prototypes) do
			global.Surfaces[index].Demand_Stops_by_Signal.fluid[fluid] = {}
			global.Surfaces[index].Supply_Stops_by_Signal.fluid[fluid] = {}
		end
	end
end
